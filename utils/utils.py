...
def edge_accuracy(preds, target, begin_steps=0, end_steps=-1):
    ...
    # for i, line in enumerate(preds[0]):
    #     print("preds:", i, line.detach().cpu().numpy().tolist())
    # for i, line in enumerate(target[0]):
    #     print("targets:", i, line.detach().cpu().numpy().tolist())
    # torch.set_printoptions(profile="default")

    preds = preds[:, begin_steps:end_steps]
    target = target[:, begin_steps:end_steps]
    correct = preds.float().data.eq(
        target.float().data.view_as(preds)).cpu().sum()
    return np.float(correct) / (target.size(0) * target.size(1) * target.size(2))
