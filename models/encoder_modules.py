...
class RNNEncoder(nn.Module):
    ...
    def edge2node(self, x, rel_rec, rel_send):
        ...
        # rel_rec.t [atom, edge(a^a-a)]
        # x         [time_step * batch, edge, hidden]
    ...
    def node2edge(self, x, rel_rec, rel_send):
        # NOTE: Assumes that we have the same graph across all samples.
        # x [time_step * batch, atom, hidden]
        # rel_rec, rel_send [edge(a^2-a), atom]
        receivers = torch.matmul(rel_rec, x)
        # receiver [time_step * batch, edge, hidden]
    ...
    def forward(self, inputs, rel_rec, rel_send):
        ...
        if self.factor:
            ...
            x = self.mlp3(x)
            # insert an RNN between two layers of Graph MLP
            ...
            x = torch.cat((x, x_skip), dim=2)  # Skip connection
            x = self.mlp4(x)
        # [num_sims, num_timesteps, num_edges, type_edges]
        # x = x.mean(0, keepdim=True).expand_as(x)
        # x = self.RNN(x, num_sims, num_timesteps, num_atoms*(num_atoms-1), x.size(-1))

        rel_type = self.fc_out(x)
        rel_type = rel_type.view([num_timesteps, num_sims, num_atoms * (num_atoms-1), -1])
        # Only use the final state of RNN
        # rel_type = rel_type[-1:].expand_as(rel_type)
        # Only use the mean pooling state of RNN
        # rel_type = rel_type.mean(0, keepdim=True).expand_as(rel_type)
        rel_type = rel_type.transpose(0, 1)
        # rel_type [batch, time_step, edge, relations]
        # rel_type = rel_type.mean(1, keepdim=True).expand_as(rel_type)
